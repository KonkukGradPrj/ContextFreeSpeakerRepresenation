...
def is_enrolled_voice(name, wav_file):
    return True
...
def STT(wav_file):
    ...
    # 음성 파일에서 텍스트 추출
    print("STT 수행 전")
    print("wav_file 의 타입 :", type(wav_file))
    print(wav_file)

    sample_rate, data = wav_file
    # 정규화 및 16비트로 변환
...
def function(name, wav_file):
    is_enrolled = is_enrolled_voice(name, wav_file)
    if is_enrolled:  # 등록된 목소리인 경우 STT
        print("등록된 목소리 입니다!")
        output_text = STT(wav_file)
        return output_text
    else:
